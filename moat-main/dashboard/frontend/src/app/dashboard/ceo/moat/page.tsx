...
export default function CeoMoatPage() {
  ...
  const fetchProjects = useCallback(async () => {
    setLoading(true);
    try {
      let res  = await fetch(`/api/ceo/projects`);
      if (res.status === 401) {
        const refreshRes = await fetch("/api/auth/refresh", { method: "POST" });
        if (refreshRes.ok) {
          res = await fetch(`/api/ceo/projects`);
        }
      }
  ...
  const saveProject = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.message || json.error || "Failed to create project");
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">My MOAT — Executive View</h1>
                    ...
                    <CardContent className="p-5 space-y-4">
                      ...
                      <div className="pt-2 border-t border-border/40 flex justify-between items-center">
                        ...
                        <Button variant="outline" size="sm" asChild className="text-xs h-7 px-3 border-[#c9a84c]/30 hover:bg-[#c9a84c]/10 hover:text-[#c9a84c]">
                          <Link href={`/dashboard/ceo/moat/${proj.id}`}>View Details &rarr;</Link>
                        </Button>
                      </div>
                    </CardContent>
                  </Card>
